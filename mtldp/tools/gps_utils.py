...
from mtldp.tools import constants as utils
...
def segment_gps_trace(lat_list, lon_list, split_into=10):
    """
    ...
    return [segment_lat_list, segment_lon_list]


def shift_geometry(geometry: dict, shift_distance: float = 7, shift_direction: str = "left"):
    """
    Shift the geometry towards a certain direction.

    :param geometry: dict that contains the latitude and longitude ``{"lat": [], "lon": []}``
    ...
    return {"lat": shifted_lat_list, "lon": shifted_lon_list}
